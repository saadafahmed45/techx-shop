...
function EditProductModal({ product, collections, onClose, onUpdated }) {
    ...
    const fetchFullProduct = async () => {
      try {
        ...
        Swal.fire({
          icon: "error",
          title: "Load Failed",
          text: "Could not load complete product details.",
        });
      } finally {
        setLoading(false);
      }
      ...
      >
        ...
        >
          <div className="flex items-center justify-between w-full mb-6 border-b border-slate-100 pb-4">
            <h2 className="text-2xl font-black text-slate-900">Loading Product Details</h2>
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-2xl bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="flex flex-col items-center justify-center py-12 flex-1">
            ...
            <p className="text-sm text-slate-400">Fetching complete product data...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[90vh] overflow-auto p-8"
        onClick={(e) => e.stopPropagation()}
      >
